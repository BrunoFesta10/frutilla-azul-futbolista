...
\vspace{5mm}
    ...
    \asegura{(\forall evento : \ent)(0< evento < \longitud{eventos}\implicaLuego (\exists recursosActualizados : \TLista{\float})(esListaDeRecursosActualizada\\(recursosActualizados, cooperan, trayectoria[evento-1], pagos, apuestas, evento) \land \\
    (\forall i : \ent)(0 \leq i < \longitud{trayectoria}\implicaLuego trayectoria[i][evento]=recursosActualizados[i]))}
    ...
    \pred{esRecursosLuegoDeApuestas}{listaAVerificar : \TLista{\float}, recursosEventoAnterior : \TLista{\float}, pagos : \TLista{\TLista{\float}}, apuestas : \TLista{\TLista{\float}}, evento :  \nat}{(\forall i : \ent)(0 \leq i < \longitud{recursosEventoAnterior} \implicaLuego listaAVerificar[i] = recursosEventoAnterior[i] * \\ pagos[individuo][eventos[evento][individuo]] * apuestas[individuo][eventos[evento][individuo]])}
...
(\exists trayectoriaSiNoCoopera : \TLista{\TLista{\float}})(esTrayectoriaValida(trayectoriaSiNoCoopera,\\setAt(cooperan,individuo,\False),pagos,apuestas,eventos,recursos)) \yLuego cooperan =\\ \IfThenElse{trayectoriaSiCoopera[individuo][\longitud{eventos}]\geq trayectoriaSiNoCoopera[individuo][\longitud{eventos}]}{\\setAt(cooperan0,individuo,\True)}{setAt(cooperan0,individuo,\False)})}

\vspace{5mm}

\pred{esTrayectoriaValida}{trayectoria:\TLista{\TLista{\float}}, cooperan : \TLista{bool}, pagos : \TLista{\TLista{\float}}, apuestas : \TLista{\TLista{\float}}, eventos : \TLista{\TLista{\float}}, recursos : \TLista{\float}}{(\longitud{trayectoria}=\longitud{recursos} \land (\forall i : \ent)(0 \leq i < \longitud{recursos} \implicaLuego \longitud{trayectoria[i]}=\longitud{eventos} \land  trayectoria[i][0]=recursos[i]) \\ \yLuego (\forall indiceEventos : \ent)(0 < indiceEvento < \longitud{eventos} \implicaLuego \\(\forall i : \ent)(0 \leq i <\longitud{trayectoria} \implicaLuego
trayectoria[i][indiceDeEventos]=  \IfThenElse{cooperan[i]=\True}{\\fondoComunEventoActual(i,e)+calculoApuestas(i,e)}{fondoComunEventoActual(i,e)}))}

\aux{fondoComunEventoActual}{FALTAN PONER LOS INPUTS}{\float}{\displaystyle\sum_{i=0}^{\longitud{trayectoria}} \IfThenElse{cooperan[i]=\True}{\\calculoApuestas(i,e)}{0})}

\aux{calculoApuestas}{FALTAN PONER LOS INPUTS}{\float}{\\trayectoria[i][eventoActual-1]*apuestas[i][eventoActual]*pagos[i][eventoActual]}
...
\yLuego trayectoria1[individuo][\longitud{eventos}] \leq trayectoria2[individuo][\longitud{eventos}] ))}
